...
## Later additions - allowing the target to float, adding font-size and saving it all with cookies
Cookies proved to be a little bit of a pain, but luckily there is a relatively popular javascript 
library called [js-cookie](https://github.com/js-cookie/js-cookie) that made creating reading cookies 
much easier. I put my parameters into a JSON array, which is then parsed with the javascript JSON functionality.
The parameters are saved in 3 variables - calcTop, calcLeft, and fontScaler.
{{< rawhtml >}}
<pre >
